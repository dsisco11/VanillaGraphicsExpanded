...
| Fixed-ray near-field, direct-world visibility and temporal helpers | Production shader owners plus existing scene binding methods. Controlled traversal-limit overrides still use the existing typed CPU uniform-buffer contract. |
...
| Site | Justification |
| --- | --- |
| `GpuProgramUseScopeTests`, `GpuProgramLayoutBindingTests`, `ShaderContractBindingTests`, `LumOnUboBindingTests`, `LumonSceneFeedbackLayoutContractTests` | Driver program, uniform, texture and block observations or deliberately authored low-level binding preconditions are the subject of these tests. |
