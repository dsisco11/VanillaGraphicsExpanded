...
## Original baseline and intended changes

At proposal approval, `GpuBindingContract` described bindings and interface locations, and `ShaderSourceLayout` applied them through VGE's GLSL AST before compilation. `ShaderStageContract` supplied shared structural defaults, deterministic binary paths and numeric specialization declarations. Runtime loading read the selected binary directly.

The original configuration model was deliberately small: structural values were binary, specialization types were strings, availability used callbacks, and the short-range AO alias was special-cased. Stage selection was spread between the contract factory, graphics programs and test helpers. Unlisted stages received an empty configuration.
...
The offline shader compiler must consume the same generated declarations without depending on the completed mod assembly. Establish and verify an acyclic clean-build pipeline; a source generator alone does not solve that dependency. A focused generator project is permitted if required by analyzer integration, but this is not a general configuration framework. Runtime accessor generation uses a shared settings-update hook; coherent stage loading, effective-change detection and replacement ownership are implemented in the runtime abstractions.
...
Generated accessors use `GpuProgram.GetShaderOption` / `SetShaderOption`. These validate and retain immutable requested settings and schedule preparation only when effective stage inputs change. A queued callback compares the latest request against installed inputs before loading; each replacement captures one coherent load plan. An isolated accessor fixture observes the settings hook without a game API or GPU allocation, while GPU lifecycle fixtures verify installation and failure preservation.

Generated scopes enumerate references directly. Compatible stages are interned by scope and identity, preserving existing shared-stage object ownership. All graphics, compute, debug/height-bake family, source-only, packaged fixture and isolated validator owners use attributes. The temporary reflection bridge, owner filter and partial-file source-linking convention are removed. Shared lighting option metadata and group declarations live beside their lighting owners and reach the offline compiler through the same semantic-input path. The typed update hook reports whether effective stage inputs changed so existing helper methods can retain their rendering stability checks. Build, source emission and runtime loading consume typed resolver selections directly; the transitional stage adapter has been removed.
