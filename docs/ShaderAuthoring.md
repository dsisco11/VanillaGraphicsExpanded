...
Generated accessors validate through the shared settings hook and request recompilation through the existing scheduling path. Inactive numeric selections remain stored and become effective when their condition becomes true. Ordinary uniforms, buffer fields and per-frame values do not belong in the option contract. Batch helpers can use the same typed `SetShaderOption` hook and its changed result when rendering must wait after an update. Resource-driven helpers may explicitly replace topology with sentinel values; conditional omission itself never discards a selected value.
