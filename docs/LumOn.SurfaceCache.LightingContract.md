...
Lighting refresh continuously revisits every eligible resident page, including pages outside a changed chunk. Fully seeded pages alternate indirect tracing and direct refresh. Partially published pages rotate indirect tracing, direct refresh and unresolved seed retries; unpublished pages prioritize seeding. Initial seed buckets retain their completion tracking. Direct and indirect refresh each have an independent cyclic texel-bucket cursor, so an unresolved bucket cannot block other buckets or the other lighting operation. Ordinary source invalidation does not restart these cursors. Page admission resumes after the last visited physical page even when eligibility temporarily disappears or changes; incompatible lifetimes and replaced page identities alone retire scheduling state.

All operations share the existing page admission budget: at most `min(RelightMaxPagesPerFrame, 65536 / tileTexels)` pages per frame, with one seed, direct-refresh or indirect operation per admitted page. Each operation samples at most `RelightTexelsPerPagePerFrame` texels per page. Reset, combination and carry-forward retain their separate 65,536-texel ceilings. For a stable set of N eligible pages, admission budget P and B texel buckets, a fully seeded page receives a complete direct and indirect attempt sweep within `2 * B * ceil(N / P)` frames. Partially published pages receive each refresh sweep within `3 * B * ceil(N / P)` frames. These are attempt bounds, not guarantees that missing inputs become available or that indirect history converges. Unavailable pages resume their cursors on recovery.

A successful direct refresh replaces only its resolved direct texels and retains ordinary visible-surface indirect history. Unknown exterior geometry leaves old values untouched. Refresh rechecks exterior occupancy even for previously hidden faces, permitting an unchanged captured face to become visible or hidden after a neighboring edit. Confirmed hidden/empty surfaces publish valid zero without emission. The previous outgoing generation remains available until the existing combine, swap and carry-forward path commits replacement output; unresolved texels retain their initialized direct/indirect values. Neither refresh admission nor completion changes `DependencyRevision`, resets world-probe tickets, or recaptures unchanged material. Pending world-probe answers still require matching geometry invalidation revisions, scheduler tickets and cache dependency identity before upload.

Implementation boundary: recurring direct and indirect scheduling is implemented; responsive temporal weighting remains the next task in `LumOn.WorldProbeSurfaceLighting.todo`. Indirect samples still use the existing capped running mean, so direct-refresh scheduling does not establish prompt indirect light-off convergence. The separate capture/indirect rejection backlog and in-game validation remain unresolved.
