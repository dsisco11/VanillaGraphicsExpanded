...
public sealed class NearFieldGeometryPartitionTests
{
    ...
    public void SourceChunkRejectsPackedCoordinateOverflow()
    {
        ...
        var positive = NearFieldGeometryPartition.SourceChunk(new(limit-1,0,0));
        positive.Decode(out int x,out _,out _); Assert.Equal((int)(limit/2-1),x);
        var negative = NearFieldGeometryPartition.SourceChunk(new(-limit,0,0));
        negative.Decode(out x,out _,out _); Assert.Equal((int)(-limit/2),x);
        Assert.Throws<ArgumentOutOfRangeException>(()=>NearFieldGeometryPartition.SourceChunk(new(limit,0,0)));
        Assert.Throws<ArgumentOutOfRangeException>(()=>NearFieldGeometryPartition.SourceChunk(new(-limit-1,0,0)));
    }
    ...
    {
        using var f = new NearFieldGeometryFixture(); f.Frame(0);
    ...
    {
        using var f = new NearFieldGeometryFixture { Available=false }; f.Frame(0);
    ...
    {
        var buffer=ArrayPool<NearFieldSourceCell>.Shared.Rent(32768);
        Array.Fill(buffer,new NearFieldSourceCell(1,default));
        var value=new NearFieldSourceCell(6,new Vector4(.1f,.2f,.3f,.4f));
        buffer[((3+16)*32+5+16)*32+7+16]=value;
        var snapshot=new NearFieldChunkSnapshot(ChunkKey.FromChunkCoords(-1,-1,-1),7,buffer.AsSpan(0,32768));
        Assert.Equal(32768L * 20, snapshot.EstimatedBytes);
        Array.Clear(buffer); ArrayPool<NearFieldSourceCell>.Shared.Return(buffer);
        var content=snapshot.Extract(new(-1,-1,-1));
        Assert.Equal(value,content.Cells[(3*16+5)*16+7]); Assert.False(content.Unsupported);
        Assert.Equal(ChunkKey.FromChunkCoords(-1,-1,-1),NearFieldGeometryPartition.SourceChunk(new(-1,-1,-1)));
        Assert.Equal(ChunkKey.FromChunkCoords(-2,0,1),NearFieldGeometryPartition.SourceChunk(new(-3,1,2)));
    ...
    {
        var first = new NearFieldChunkSnapshot(default,1,new NearFieldSourceCell[32768]);
        var secondKey = ChunkKey.FromChunkCoords(1,0,0);
        var second = new NearFieldChunkSnapshot(secondKey,1,new NearFieldSourceCell[32768]);
    ...
    {
        using var cache = new NearFieldChunkCache((key,version,cancel) => Task.FromResult<NearFieldChunkSnapshot?>(
            new(ChunkKey.FromChunkCoords(1,0,0),version,new NearFieldSourceCell[32768])), _=>1, _=>true);
    ...
    {
        using var f = new NearFieldGeometryFixture();
    ...
    {
        using var f=new NearFieldGeometryFixture(maximumInFlight:1){Deferred=true};
    ...
    {
        using var f=new NearFieldGeometryFixture(capturesPerFrame:1);
