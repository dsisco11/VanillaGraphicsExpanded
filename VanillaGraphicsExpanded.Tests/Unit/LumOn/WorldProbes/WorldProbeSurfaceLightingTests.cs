...
public sealed class WorldProbeSurfaceLightingTests
{
    ...
    /// <summary>Later worker and resolver calls cannot mutate batches already transferred to their consumers.</summary>
    [Fact]
    public void TransferredBatchesRemainStableAcrossSubsequentTracingAndResolution()
    {
        ...
        var source=integrator.TraceProbe(world.CreateTraceScene(),work,CancellationToken.None);
        var original=source.AtlasSamples.ToArray();
        var answers=source.AtlasSamples.Select(sample=>sample.SurfaceHit!.Value).ToArray();
        for(int i=0;i<answers.Length;i++) answers[i].Result=new(2,3,4,1);
        int index=0;
        var first=WorldProbeSurfaceLighting.Resolve(source,answers,ref index);
        var firstPublished=first.AtlasSamples.ToArray();
        integrator.TraceProbe(world.CreateTraceScene(),work with { FrameIndex=1,WorldProbeAtlasTexelsPerUpdate=64 },CancellationToken.None);
        Array.Clear(answers);
        index=0;var second=WorldProbeSurfaceLighting.Resolve(source,answers,ref index);
        Assert.False(second.Success);Assert.Equal(4,second.RetrySamples.Length);
        Assert.Equal(original,source.AtlasSamples.ToArray());
        Assert.Equal(firstPublished,first.AtlasSamples.ToArray());
        Assert.All(first.AtlasSamples,sample=>Assert.Equal(new Vector3(2,3,4),sample.RadianceRgb));
        Assert.All(source.AtlasSamples,sample=>Assert.NotNull(sample.SurfaceHit));
    }

    /// <summary>A default immutable payload is safely treated as no publishable samples.</summary>
    [Fact]
    public void DefaultTransferredBatchIsEmptyAndUnavailable()
    {
        LumOnWorldProbeTraceResult source=default;
        int index=0;var result=WorldProbeSurfaceLighting.Resolve(source,[],ref index);
        Assert.False(result.Success);Assert.True(result.AtlasSamples.IsDefaultOrEmpty);
        Assert.True(result.RetrySamples.IsDefaultOrEmpty);Assert.Equal(0,index);
    }
    ...
    public void MixedAnswersPreserveReadyDirectionsAndRetryOnlyUnresolvedHits()
    {
        ...
        Assert.True(traced.AtlasSamples.AsSpan()[2..].SequenceEqual(partial.RetrySamples.AsSpan()));
        ...
        Assert.True(complete.RetrySamples.IsDefaultOrEmpty);
    ...
    public void MissingAnswersProduceOnlyRetryDescriptors()
    {
        ...
        int index=0;var result=WorldProbeSurfaceLighting.Resolve(source,[],ref index);
        Assert.False(result.Success);Assert.Empty(result.AtlasSamples);
        Assert.True(source.AtlasSamples.AsSpan().SequenceEqual(result.RetrySamples.AsSpan()));
    }
    #endregion
