...
public sealed partial class LumOnLocalTraceFunctionalTests
{
    ...
    /// <summary>Adding visible geometry must not erase outgoing radiance supplied by the same local scene off screen.</summary>
    [Theory]
    [InlineData(0f)]
    [InlineData(0.25f)]
    [InlineData(1f)]
    public void ScreenHits_PreserveNonEmissiveLocalLighting(float lighting)
    {
        ...
        var screenOnly = Trace(fixture, screenDepth: depth, localTracing: false);
    ...
    /// <summary>Shared local shading counts emission once; unavailable local data preserves explicit visible emission.</summary>
    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    public void ScreenHits_EmissionIsNotDoubled_AndSurvivesUnavailableLocalData(bool publish)
    {
        ...
        float depth = ScreenDepthAt(-7);
        var screenOnly = Trace(fixture, screenDepth: depth, localTracing: false, screenEmission: 0.25f, emissionBoost: 2);
    ...
    public void ScreenHits_MissingLocalMaterial_DoesNotBorrowScreenEmission()
    {
        ...
        float depth = ScreenDepthAt(-7);
        var screenOnly = Trace(fixture, screenDepth: depth, localTracing: false, screenEmission: 1);
