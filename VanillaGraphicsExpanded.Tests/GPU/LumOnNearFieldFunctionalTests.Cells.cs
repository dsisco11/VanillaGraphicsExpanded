...
public sealed partial class LumOnNearFieldFunctionalTests
{
    ...
    public void ProductionPartition_PublishesAndInvalidatesGpuSubcells()
    {
        ...
        using var scene = new NearFieldGpuScene(32);
        scene.SetWindow(default);
        int version = 1;
        var source = Enumerable.Repeat(new NearFieldSourceCell(1, Vector4.Zero),32768).ToArray();
        var delayed = new List<(int Version, TaskCompletionSource<NearFieldChunkSnapshot?> Completion)>();
        using var cache = new NearFieldChunkCache((key, revision, cancellation) =>
        {
            if (revision == 1) return Task.FromResult<NearFieldChunkSnapshot?>(new(key,revision,source));
            var completion = new TaskCompletionSource<NearFieldChunkSnapshot?>();
            delayed.Add((revision,completion));
            return completion.Task;
        }, _ => version, _ => true);
        var provider = new NearFieldGeometryPartition(cache,scene,new NearFieldMaterialRegistry());
    ...
    {
        EnsureShaderTestAvailable();
        using var scene = new NearFieldGpuScene(16);
        scene.SetWindow(default);
        var key = new PartitionCellKey(1, "test", new(0, 0, 0));
        var request = new PartitionRequest(1, key, 1, 1, 1, CancellationToken.None);
        var cells = Enumerable.Repeat(new NearFieldSourceCell(1, Vector4.Zero), 4096).ToArray();
        ...
        Assert.Equal(new byte[] { 0 }, ReadReadiness(scene));
        Assert.False(scene.PublishCell(request, cells, new NearFieldMaterialRegistry()));
    }
