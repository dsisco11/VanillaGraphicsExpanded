...
[Collection("GPU")]
[Trait("Category", "GPU")]
public sealed class LumOnLocalGeometryDebugFunctionalTests : LumOnShaderFunctionalTestBase
{
    #region Construction
    /// <summary>Uses the shared mandatory OpenGL context.</summary>
    public LumOnLocalGeometryDebugFunctionalTests(HeadlessGLFixture fixture) : base(fixture) { }
    #endregion
    ...
    public void GeometryView_CameraMovesAcrossVoxelBoundary_AtLargeOrigins(int offset)
    {
        ...
        using var fixture = new LocalTraceVoxelFixture(new VectorInt3(offset, 0, 0));
    ...
    private float[] RenderGeometry(LocalTraceGpuScene? scene, Vector3? camera = null, Vector3d? playerOrigin = null, bool monolithic = false)
    {
        ...
        try
        {
            ...
            using var parameters = new ObjectParamsUbo("Tests.LocalGeometryDebug");
            ...
            GL.Uniform1(GL.GetUniformLocation(program, "localTraceGeometry"), 34);
            GL.Uniform1(GL.GetUniformLocation(program, "localTraceRegions"), 35);
            GL.UseProgram(0);
