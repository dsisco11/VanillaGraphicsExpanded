...
[Collection("LocalTraceMaterialCapture")]
[Trait("Category", "GPU")]
public sealed class LumOnLocalTraceMaterialReadinessTests : LocalTraceShaderTestBase
{
    #region Construction
    /// <summary>Uses an exclusive GPU context while temporarily changing the singleton registry.</summary>
    public LumOnLocalTraceMaterialReadinessTests(HeadlessGLFixture fixture) : base(fixture) { }
    #endregion
    ...
    public void CapturedBeforeMaterialReadiness_StaysUnresolvedUntilRecaptured(bool surfaceReady, bool derivedReady)
    {
        ...
        var captured = LocalTraceCellCapture.Capture(ControlledBlockAccessor.Create(world), material.Cube,
            new BlockPos(-3, 0, -5), new LocalTraceMaterialRegistry());
    ...
    private static void AssertPublished(LocalTraceGpuScene scene)
    {
