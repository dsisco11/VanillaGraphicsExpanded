#ifndef LUMON_LOCAL_HIT_LIGHTING_GLSL
#define LUMON_LOCAL_HIT_LIGHTING_GLSL
...
bool lumonShadeLocalHit(LumonLocalHit hit, float traceDistance, float emissionBoost, out vec3 radiance)
{
    radiance = vec3(0.0);
    ...
    vec4 material = texelFetch(localTraceMaterials, ivec2(materialTexel % 256, materialTexel / 256), 0);
    int emissionTexel = materialTexel + 1;
    vec3 emission = texelFetch(localTraceMaterials, ivec2(emissionTexel % 256, emissionTexel / 256), 0).rgb;
    uint outsideGeometry; vec4 outsideLight;
    if (!lumonLocalRead(hit.cell + hit.normal, outsideGeometry, outsideLight)) return false;
    float skyFactor = 0.0;
    if (outsideLight.a > 1e-6)
    {
        ...
        for (int sampleIndex = 0; sampleIndex < 2; sampleIndex++)
        {
            ...
            if (weight > 0.0 && lumonTraceLocal(hit.cell + offset, fract(start), skyDirection, min(traceDistance, 16.0)).outcome == LUMON_LOCAL_CLEAR)
                skyFactor += weight;
        }
    }
