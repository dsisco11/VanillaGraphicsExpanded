#ifndef LUMON_LOCAL_CACHE_HANDOFF_GLSL
#define LUMON_LOCAL_CACHE_HANDOFF_GLSL

/** Selects a finite cache neighborhood and the segment that must be proven clear before interpolation. */
bool lumonLocalCacheCoverage(vec3 position, out int level, out float radius, out float traceDistance)
{
    ...
    // Every neighboring center lies within one cell diagonal. The local segment
...
/** Samples only distant cache texels after local geometry has been resolved; no probe-to-surface rejection. */
