...
/** Implements render lumon scene material atlas all roughness debug for its explicit view entrypoint. */
...
/** Renders only the LumonSceneMaterialAtlasAllRoughness view; mode selection occurs before program loading. */
void main()
{
    uv = gl_FragCoord.xy / screenSize;
    vec2 screenPos = uv * screenSize;
    outColor = renderLumonSceneMaterialAtlasAllRoughnessDebug();
}
