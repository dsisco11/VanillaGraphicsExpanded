...
internal sealed class LocalGeometryPartition : IPartitionProvider
{
    private readonly ILocalTraceChunkSource source;
    private readonly ILocalTracePublicationBackend backend;
    ...
    public LocalGeometryPartition(ILocalTraceChunkSource source, ILocalTracePublicationBackend backend, LocalTraceMaterialRegistry materials)
    {
    ...
    public PartitionCapture Capture(PartitionRequest request)
    {
        ...
        if (!source.TryGet(key, out LocalTraceChunkSnapshot? snapshot) || snapshot == null)
            return new(PartitionContentStatus.MissingDependencies, null);
    ...
    {
        var chunk = (LocalTraceChunkSnapshot)snapshot;
        LocalTraceCellContent content = chunk.Extract(request.Key.Coordinate);
        // Capture can discover more materials before publication; reserve the worst-case palette transfer.
        complete(new(request, snapshot, content, 4096L * (sizeof(uint) + 4 * sizeof(float)) + 1 + LocalTraceMaterialRegistry.MaximumUploadBytes,
    ...
    public bool Publish(PartitionCompletion completion) => backend.ClaimCell(completion.Request) &&
        backend.PublishCell(completion.Request, ((LocalTraceCellContent)completion.Content!).Cells, materials);
