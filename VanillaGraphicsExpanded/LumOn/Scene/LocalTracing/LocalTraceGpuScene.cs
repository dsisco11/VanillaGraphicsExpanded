...
internal sealed class LocalTraceGpuScene : IDisposable, ILocalTracePublicationBackend
{
    ...
    public LocalTraceGpuScene(int requestedResolution, int cellSize = 16)
    {
    ...
    public void SetWindow(in VectorInt3 origin)
    {
        if (origin.X % CellSize != 0 || origin.Y % CellSize != 0 || origin.Z % CellSize != 0)
            throw new ArgumentException("Local window must align to publication cells.");
    ...
    public bool PublishCell(PartitionRequest request, ReadOnlySpan<LocalTraceSourceCell> cells, LocalTraceMaterialRegistry materials)
    {
        if (request.Cancellation.IsCancellationRequested || !ContainsCell(request.Key.Coordinate)) return false;
        ...
        {
            LocalTraceSourceCell c = cells[(y * CellSize + z) * CellSize + x];
