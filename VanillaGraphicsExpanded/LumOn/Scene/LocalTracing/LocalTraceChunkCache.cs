...
internal sealed class LocalTraceChunkCache : ILocalTraceChunkSource, IDisposable
{
    /// <summary>One source request retains cancellation ownership until all workers acknowledge it.</summary>
    private sealed record Entry(int Version, CancellationTokenSource Cancellation, Task<LocalTraceChunkSnapshot?> Task, long Frame);
    private readonly Func<ChunkKey, int, CancellationToken, Task<LocalTraceChunkSnapshot?>> load;
    ...
    public LocalTraceChunkCache(Func<ChunkKey, int, CancellationToken, Task<LocalTraceChunkSnapshot?>> load,
    ...
    public bool TryGet(ChunkKey key, out LocalTraceChunkSnapshot? snapshot)
    {
        snapshot = null;
        if (!available(key))
        {
        ...
        Task<LocalTraceChunkSnapshot?> task = load(key, current, cancellation.Token);
