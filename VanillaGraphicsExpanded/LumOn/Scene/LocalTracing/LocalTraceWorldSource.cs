...
internal sealed class LocalTraceWorldSource : ILocalTraceChunkSource, IDisposable
{
    private readonly ICoreClientAPI capi;
    private readonly LumonSceneTraceSceneChunkVersionProvider versions = new();
    private readonly LocalTraceChunkProcessor processor = new();
    private readonly ChunkProcessingService processing;
    private readonly LocalTraceChunkCache cache;
    private readonly LocalTraceSourceLifetimes lifetimes;
    ...
    public LocalTraceWorldSource(ICoreClientAPI capi, LocalTraceMaterialRegistry materials)
    {
        this.capi = capi;
        ...
        var snapshots = new LumonSceneTraceSceneChunkSnapshotSource(capi, versions,
            ...
            materials, localOnly: true);
        processing = new ChunkProcessingService(snapshots, versions, new() { WorkerCount = 2 });
        cache = new LocalTraceChunkCache(LoadChunk, versions.GetCurrentVersion, IsAvailable);
    }
    ...
    public bool TryGet(ChunkKey key, out LocalTraceChunkSnapshot? snapshot) => cache.TryGet(key, out snapshot);
    ...
    private async Task<LocalTraceChunkSnapshot?> LoadChunk(ChunkKey key, int version, CancellationToken cancellation)
    {
        ChunkWorkResult<LocalTraceChunkSnapshot> result = await processing.RequestAsync(key, version, processor, ct: cancellation).ConfigureAwait(false);
