...
/// <summary>Bounded origin domain and conservative trace reach for independent local geometry coverage.</summary>
internal sealed record LocalTraceCoveragePolicy(double OriginRadius = 16, double PrefetchMargin = 16, int MaximumResolution = 192)
{
    ...
    public bool TryPlan(in PartitionPoint position, double traceReach, out LocalTraceCoveragePlan plan)
    {
        plan = default;
...
internal readonly record struct LocalTraceCoveragePlan(PartitionBounds Required, PartitionBounds Origins, VectorInt3 WindowOrigin, int Resolution, double TraceReach);
