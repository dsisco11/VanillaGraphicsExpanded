...
internal sealed class NearFieldChunkSnapshot : IPartitionSnapshot, IArtifactSizeInfo
{
    private readonly NearFieldSourceCell[] cells;
    public ChunkKey Key { get; }
    public int Version { get; }
    public long EstimatedBytes => (long)cells.Length * System.Runtime.CompilerServices.Unsafe.SizeOf<NearFieldSourceCell>();

    /// <summary>Copies source data so pooled snapshot disposal cannot change a pending publication.</summary>
    public NearFieldChunkSnapshot(ChunkKey key, int version, ReadOnlySpan<NearFieldSourceCell> source)
    {
    ...
    public NearFieldCellContent Extract(in PartitionCoordinate coordinate)
    {
        ...
        var result = new NearFieldSourceCell[4096];
        bool unsupported = false;
        for (int y = 0; y < 16; y++)
        for (int z = 0; z < 16; z++)
        for (int x = 0; x < 16; x++)
        {
            NearFieldSourceCell cell = cells[((y + oy) * 32 + z + oz) * 32 + x + ox];
